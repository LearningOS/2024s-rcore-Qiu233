...
pub fn sys_mutex_create(blocking: bool) -> isize {
    ...
    if let Some(id) = process_inner
        ...
        .map(|(id, _)| id)
    {
        process_inner.mutex_lock.locks[id] = mutex;
        if process_inner.deadlock_detection {
            process_inner.mutex_lock.prepare_lock_state(tid + 1, id + 1);
            process_inner.mutex_lock.avail[id] = 1;
        }
        id as isize
    } else {
        process_inner.mutex_lock.locks.push(mutex);
        let id = process_inner.mutex_lock.locks.len() - 1;
        if process_inner.deadlock_detection {
            process_inner.mutex_lock.prepare_lock_state(tid + 1, id + 1);
            process_inner.mutex_lock.avail.push(1);
        }
        id as isize
    }
}
...
pub fn sys_mutex_lock(mutex_id: usize) -> isize {
    ...
    let mut process_inner = process.inner_exclusive_access();
    if process_inner.deadlock_detection {
        process_inner.mutex_lock.prepare_lock_state(tid + 1, mutex_id + 1);
    }
    let mutex = Arc::clone(process_inner.mutex_lock.locks[mutex_id].as_ref().unwrap());
    if process_inner.deadlock_detection && process_inner.mutex_lock.has_deadlock_if_wait_for(tid, mutex_id) {
        return -0xDEAD;
    }
    if process_inner.deadlock_detection {
        process_inner.mutex_lock.pre_wait(tid, mutex_id);
    }
    drop(process_inner);
    mutex.lock();
    if process.inner_exclusive_access().deadlock_detection {
        process.inner_exclusive_access().mutex_lock.post_wait_succ(tid, mutex_id);
    }
    0
}
...
pub fn sys_mutex_unlock(mutex_id: usize) -> isize {
    ...
    let mutex = Arc::clone(process_inner.mutex_lock.locks[mutex_id].as_ref().unwrap());
    if process_inner.deadlock_detection {
        process_inner.mutex_lock.pre_release(tid, mutex_id);
    }
    drop(process_inner);
    mutex.unlock();
    let process = current_process();
    let mut process_inner = process.inner_exclusive_access();
    if process_inner.deadlock_detection {
        process_inner.mutex_lock.post_release(tid, mutex_id);
    }
    0
}
